...
namespace {
auto deleteSeq = [](auto ptr, auto controlBlock) {
    if (controlBlock) {
        controlBlock->decrementSharedRefs();
        if (controlBlock->getSharedRefs() == 0 && controlBlock->getWeakRefs() == 0) {
            controlBlock->defaultDeleter(ptr);
            delete controlBlock;
        }
    }
};
